...
Games = read_html("https://afltables.com/afl/seas/2017.html")
...
for(i in 1:length(Games.list)){
  ...
  for(j in 1:nrow(Games.list[[i]])){
    ...
    HomeStats <- html_table(HomeStatsNodes, header = TRUE, fill = TRUE)[[k1 - 2]]
...
setwd("C:/Users/Ben/Documents/AFL2017")

for(i in 1:length(Umpire.list)){
  for(j in 1:length(Umpire.list[[i]])){
    write.csv(Umpire.list[[i]][[j]], file = paste0("AFL2017_Umpire_Round", ifelse(nchar(i) == 1, paste0("0", i), i),
